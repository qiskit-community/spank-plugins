...
impl Client {
    /// Returns the pulse defaults for the specified backend.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///         .unwrap();
    ///     let _config = client
    ///         .get_backend_pulse_defaults::<serde_json::Value>("ibm_brisbane")
    ///         .await?;
    ///     Ok(())
    /// }
    /// ```
    ...
    pub async fn get_backend_pulse_defaults<T: DeserializeOwned>(
        &self,
        backend_name: &str,
    ) -> Result<T> {
