...
* The environment variables required for execution are set by the [spank_qrmi](../../plugins/spank_qrmi) at runtime. Users can override the settings by setting the environment variables({backend_name}_QRMI_xxxx) listed in the following sections when executing the `sbatch` command.
